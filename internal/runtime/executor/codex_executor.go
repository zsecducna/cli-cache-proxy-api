...
const (
	codexUserAgent             = "codex-tui/0.128.0 (Mac OS 26.3.1; arm64) iTerm.app/3.6.9 (codex-tui; 0.128.0)"
...
func stripCodexUnsupportedRequestFields(body []byte) []byte {
...
func translateOriginalAndRequest(from, to sdktranslator.Format, model string, originalPayload, requestPayload []byte, stream bool) ([]byte, []byte) {
	originalTranslated := sdktranslator.TranslateRequest(from, to, model, originalPayload, stream)
	if bytes.Equal(originalPayload, requestPayload) {
		return originalTranslated, bytes.Clone(originalTranslated)
	}
	return originalTranslated, sdktranslator.TranslateRequest(from, to, model, requestPayload, stream)
}
...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	originalTranslated, body := translateOriginalAndRequest(from, to, baseModel, originalPayload, req.Payload, false)
	...
	httpReq, cacheSelection, err := e.cacheHelper(ctx, from, url, req, body)
	if err != nil {
		return resp, err
	}
	body = cacheSelection.Body
	...
	for _, line := range lines {
		...
		translatePayload := completedData
		if len(outputItemsByIndex) == 0 && len(outputItemsFallback) == 0 {
			translatePayload = data
		}

		var param any
		out := sdktranslator.TranslateNonStream(ctx, to, from, req.Model, originalPayload, body, translatePayload, &param)
...
func (e *CodexExecutor) executeCompact(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	helps.SetCodexCacheResponseObservability(ctx, data, cacheSelection.Key)
	helps.AppendAPIResponseChunk(ctx, e.cfg, data)
...
func (e *CodexExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	originalPayload := originalPayloadSource
	originalTranslated, body := translateOriginalAndRequest(from, to, baseModel, originalPayload, req.Payload, true)
...
func (e *CodexExecutor) cacheHelper(ctx context.Context, from sdktranslator.Format, url string, req cliproxyexecutor.Request, rawJSON []byte) (*http.Request, codexPromptCacheSelection, error) {
	...
	for key, values := range selection.Hdrs {
		for _, value := range values {
			httpReq.Header.Add(key, value)
		}
	}
	return httpReq, selection, nil
}
