...
// ApplyPayloadConfigWithRequest applies payload config with source protocol and
// request header gates while preserving root-relative payload paths.
...
// resolvePayloadRulePaths expands gjson query path segments such as
// `tools.#(type=="function")#.name` into concrete sjson paths so payload rules
// can mutate every matching element deterministically.
