...
import (
	...
	codexhandlers "github.com/router-for-me/CLIProxyAPI/v7/sdk/api/handlers/codex"
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if errConfigureUsage := usage.ConfigurePersistentStore(cfg, configFilePath); errConfigureUsage != nil {
		log.Errorf("failed to configure persistent cache statistics store: %v", errConfigureUsage)
	}
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		// Keep the upstream Redis-compatible usage drain endpoint available for cheapRouter ingestion.
...
// handleHomeCodexClientModels returns Home-provided models in the Codex client
// model-list shape used when clients pass the `client_version` query flag.
...
// handleHomeModels adapts Home model payloads to OpenAI or Claude model-list
// shapes so Home mode keeps provider-specific client compatibility.
func (s *Server) handleHomeModels(c *gin.Context) {
	...
	if isClaude {
		...
		for _, entry := range entries {
			model := map[string]any{"id": entry.id, "object": "model", "owned_by": entry.ownedBy}
			if entry.created > 0 {
		...
		c.JSON(http.StatusOK, gin.H{"data": out, "has_more": false, "first_id": firstID, "last_id": lastID})
		return
	}
...
func (s *Server) handleHomeGeminiModels(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"models": formatHomeGeminiModels(entries)})
}
...
func (s *Server) handleHomeGeminiModel(c *gin.Context) {
	...
	c.JSON(http.StatusNotFound, handlers.ErrorResponse{
		Error: handlers.ErrorDetail{Message: "Not Found", Type: "not_found"},
	})
}
...
func (s *Server) loadHomeModelEntries(c *gin.Context) ([]homeModelEntry, bool) {
	...
	if client == nil {
		c.JSON(http.StatusServiceUnavailable, handlers.ErrorResponse{
			Error: handlers.ErrorDetail{Message: "home control center unavailable", Type: "server_error"},
		})
		return nil, false
	}
	...
	if errGet != nil {
		c.JSON(http.StatusBadGateway, handlers.ErrorResponse{
			Error: handlers.ErrorDetail{Message: errGet.Error(), Type: "server_error"},
		})
		return nil, false
	}
	...
	if errDecode != nil {
		c.JSON(http.StatusBadGateway, handlers.ErrorResponse{
			Error: handlers.ErrorDetail{Message: errDecode.Error(), Type: "server_error"},
		})
		return nil, false
	}
	return entries, true
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if errConfigureUsage := usage.ConfigurePersistentStore(cfg, s.configFilePath); errConfigureUsage != nil {
...
func AuthMiddleware(manager *sdkaccess.Manager) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err == nil {
			if result != nil {
				// Preserve both context keys because older fork paths and tests still read apiKey directly.
