...
func TestManagementControlPanelIncludesCacheStatisticsIntegration(t *testing.T) {
	...
	if !strings.Contains(body, "renderUsageModelBreakdownCharts") || !strings.Contains(body, "Token Type Breakdown") || !strings.Contains(body, "Cost Overview") || !strings.Contains(body, "Set model prices to view per-model cost overview.") || !strings.Contains(body, "buildSeriesChartSVG") || !strings.Contains(body, "formatModelAxisLabel") || !strings.Contains(body, "long_context_input_tokens") || !strings.Contains(body, "supportsOpenAILongContextPricing") || !strings.Contains(body, "OPENAI_LONG_CONTEXT_PROMPT_MULTIPLIER") || !strings.Contains(body, "OPENAI_LONG_CONTEXT_COMPLETION_MULTIPLIER") {
		t.Fatalf("management enhancer should replace the remaining aggregate usage breakdown cards with model-based token and cost charts driven by cache statistics summaries, including OpenAI long-context pricing adjustments: %s", body)
	}
