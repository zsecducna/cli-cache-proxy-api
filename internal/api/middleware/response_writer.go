...
func (w *ResponseWriterWrapper) Finalize(c *gin.Context) error {
	...
	// Keep temp-backed websocket sections consistent even in error-only mode:
	// we still need to clean up file-backed sources when logging is skipped, and
	// forced logging should trigger only for real upstream proxy failures.
	...
	if w.isStreaming && w.streamWriter != nil {
		...
		if len(slicesAPIResponseError) > 0 {
			_ = w.streamWriter.WriteAPIResponseErrors(slicesAPIResponseError)
		}
