...
type StreamingLogWriter interface {
	...
	// WriteAPIResponseErrors writes upstream API error responses to the log.
	// This should be called when upstream error responses were captured separately from the main response.
	//
	// Parameters:
	//   - apiResponseErrors: Captured upstream API error responses
	//
	// Returns:
	//   - error: An error if writing fails, nil otherwise
	WriteAPIResponseErrors(apiResponseErrors []*interfaces.ErrorMessage) error
...
// inferUpstreamTransport treats explicit API error envelopes as HTTP traffic
// even when the upstream body is absent, so forced-error logs still show that
// the proxy reached an upstream HTTP path before failover or disconnect.
func inferUpstreamTransport(apiRequest, apiResponse, apiWebsocketTimeline []byte, apiWebsocketTimelineSource *FileBodySource, apiResponseErrors []*interfaces.ErrorMessage) string {
	hasHTTP := hasSectionPayload(apiRequest) || hasSectionPayload(apiResponse) || len(apiResponseErrors) > 0
...
type FileStreamingLogWriter struct {
	...
	// apiResponseErrors stores upstream API error responses.
	apiResponseErrors []*interfaces.ErrorMessage
...
func (w *FileStreamingLogWriter) WriteAPIResponseErrors(apiResponseErrors []*interfaces.ErrorMessage) error {
	...
	for i := 0; i < len(apiResponseErrors); i++ {
		...
		if apiResponseErrors[i].Error != nil {
			copied.Error = fmt.Errorf("%s", apiResponseErrors[i].Error.Error())
		}
		if len(apiResponseErrors[i].Addon) > 0 {
			copied.Addon = apiResponseErrors[i].Addon.Clone()
		}
...
func (w *FileStreamingLogWriter) writeFinalLog(logFile *os.File) error {
	if errWrite := writeRequestInfoWithBody(logFile, w.url, w.method, w.requestHeaders, nil, w.requestBodyPath, w.timestamp, "http", inferUpstreamTransport(w.apiRequest, w.apiResponse, w.apiWebsocketTimeline, nil, w.apiResponseErrors), true); errWrite != nil {
		return errWrite
	}
...
func (w *NoOpStreamingLogWriter) WriteAPIResponseErrors(_ []*interfaces.ErrorMessage) error {
	return nil
}
...
// WriteAPIResponseErrors keeps upstream failure details available for home-mode
// request logs so streaming and non-streaming log paths expose the same
// diagnostics to PostgreSQL-backed readers.
func (w *homeStreamingLogWriter) WriteAPIResponseErrors(apiResponseErrors []*interfaces.ErrorMessage) error {
...
func (w *homeStreamingLogWriter) Close() error {
	...
	var buf bytes.Buffer
	upstreamTransport := inferUpstreamTransport(w.apiRequest, w.apiResponse, w.apiWebsocketTime, nil, w.apiResponseErrors)
