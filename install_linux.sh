...
require_tools() {
  local missing=0
  local tool=""
  for tool in bash git go sqlite3 systemctl; do
...
write_systemd_unit() {
  ...
  local binary_path="$2"
  local config_path="$3"
  local unit_path="$HOME/.config/systemd/user/${INSTALLER_LABEL}.service"
...
Restart=always
RestartSec=5
StandardOutput=append:$install_root/logs/systemd.stdout.log
StandardError=append:$install_root/logs/systemd.stderr.log
...
stop_systemd_service() {
  ...
  systemctl --user stop "$INSTALLER_LABEL" >/dev/null 2>&1 || true
}
...
start_systemd_service() {
  ...
  if [[ "$SKIP_SYSTEMD" == "1" ]]; then
    say "Skipping systemctl --user calls because CLI_PROXY_INSTALLER_SKIP_SYSTEMD=1"
    return 0
  fi

  if ! systemctl --user daemon-reload; then
    warn "systemctl --user daemon-reload failed. Installation will continue; use the manual recovery commands below."
    return 1
  fi
  if ! systemctl --user enable "$(basename "$unit_path")"; then
    warn "systemctl --user enable failed for $unit_path. Installation will continue; use the manual recovery commands below."
    return 1
  fi
  if ! systemctl --user restart "$INSTALLER_LABEL"; then
    warn "systemctl --user restart failed for $INSTALLER_LABEL. Installation will continue; use the manual recovery commands below."
    return 1
  fi
}
...
print_next_steps() {
  ...
  if [[ -n "$unit_path" ]]; then
    ...
    say "    systemctl --user enable $(basename "$unit_path")"
    say "    systemctl --user restart $INSTALLER_LABEL"
...
main() {
  ...
  if [[ "$build_now" -eq 1 ]]; then
    ...
    die "Build was skipped, but the existing binary at $binary_path would hide source changes. Re-run install_linux.sh and choose to build from source so UI changes are installed."
  else
  ...
  if [[ "$create_service" -eq 1 ]]; then
    stop_systemd_service
    unit_path="$(write_systemd_unit "$install_root" "$binary_path" "$config_path")"
    if [[ "$start_service" -eq 1 ]]; then
      start_systemd_service "$unit_path" || true
    fi
  fi
