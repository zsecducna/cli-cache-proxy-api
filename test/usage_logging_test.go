...
// TestGeminiExecutorRecordsSuccessfulZeroUsageInQueue guards zero-token usage records
// so queue-based statistics do not drop legitimate successful requests.
...
// waitForQueuedUsageModelTotalTokens polls the in-memory Redis queue facade until
// the expected provider/model usage payload is published by the usage plugin.
...
// queuedUsagePayload mirrors the fields needed from the queue plugin JSON
// without coupling this test to the full internal payload shape.
...
// parseQueuedUsagePayload tolerates unrelated queue entries while preserving
// strict validation for the provider/model payload this test is waiting for.
